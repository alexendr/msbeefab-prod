...
if ( !class_exists( 'Kuteshop_Woo_Functions' ) ) {
	class Kuteshop_Woo_Functions
	{
		...
		function action_product_woocommerce()
		{
			...
			add_action( 'woocommerce_before_main_content', array( $this, 'kuteshop_woocommerce_before_main_content' ), 10 );
			add_action( 'woocommerce_after_main_content', array( $this, 'kuteshop_woocommerce_after_main_content' ), 50 );
			add_action( 'woocommerce_before_main_content', array( $this, 'kuteshop_contain_before_shop_loop' ), 50 );
			add_action( 'woocommerce_after_main_content', array( $this, 'kuteshop_contain_after_shop_loop' ), 10 );
			add_action( 'woocommerce_before_shop_loop', function () { echo '<div class="row equal-container better-height">'; }, 50 );
			add_action( 'woocommerce_after_shop_loop', function () { echo '</div>'; }, 50 );
			...
			add_action( 'woocommerce_before_shop_loop', array( $this, 'kuteshop_before_shop_control' ), 20 );
			add_action( 'woocommerce_after_shop_loop', array( $this, 'kuteshop_after_shop_control' ), 50 );
			/**
			 * CUSTOM PRODUCT RATING
			 */
			add_filter( 'woocommerce_product_get_rating_html', array( $this, 'kuteshop_product_get_rating_html' ), 10, 3 );
		}
		...
		function kuteshop_contain_before_shop_loop()
		{
			/*Shop layout*/
			...
			if ( !is_product() )
				$this->kuteshop_shop_banner();
		}

		function kuteshop_contain_after_shop_loop()
		{
			echo '</div>';
		}
		...
		function product_display_mode_request()
		{
			if ( isset( $_POST['display_mode_action'] ) ) {
				wp_redirect(
					add_query_arg(
						array(
							'shop_page_layout' => $_POST['display_mode_value'],
