	...
	class Breadcrumb_Trail
	{
		...
		public function trail()
		{
			...
			if ( 0 < $item_count ) {
				...
				$breadcrumb .= '<ul class="trail-items breadcrumb" itemscope itemtype="' . esc_url( 'http://schema.org/BreadcrumbList' ) . '">';
				...
				foreach ( $this->items as $item ) {
					...
					$attributes = 'itemprop="itemListElement" itemscope itemtype="' . esc_url( 'http://schema.org/ListItem' ) . '" class="' . $item_class . '"';
		...
		)
		{
			...
			if ( is_array( $matches ) ) {
				...
				foreach ( $matches as $match ) {
					...
					if ( '%year%' == $tag )
						$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'kuteshop' ) ) ) );
					// If using the %monthnum% tag, add a link to the monthly archive.
					elseif ( '%monthnum%' == $tag )
						$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'kuteshop' ) ) ) );
					// If using the %day% tag, add a link to the daily archive.
					elseif ( '%day%' == $tag )
						$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'kuteshop' ) ) ) );
