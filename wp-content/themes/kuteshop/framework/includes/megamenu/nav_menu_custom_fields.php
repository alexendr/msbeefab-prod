...
add_action( 'walker_nav_menu_custom_fields', 'kuteshop_add_custom_fields', 10, 4 );
function kuteshop_add_custom_fields( $item_id, $item, $depth, $args ) {
    ...
    <div class="container-megamenu container-<?php echo esc_attr( $depth ); ?>">
        <p class="item_icon_type">
            <label class="row_label"><strong><?php esc_html_e( 'Icon Settings' ,'kuteshop' );?></strong></label><br />
            <input <?php checked( 'none', $item_icon_type, true ); ?> name="menu-item-megamenu-item_icon_type[<?php echo esc_attr( $item_id ); ?>]" id="none_item_icon_type_<?php echo esc_attr( $item_id );?>" type="radio" value="none"><label for="none_item_icon_type_<?php echo esc_attr( $item_id );?>"><?php esc_html_e('None' , 'kuteshop') ?></label>
            <input <?php checked( 'fonticon', $item_icon_type, true ); ?> name="menu-item-megamenu-item_icon_type[<?php echo esc_attr( $item_id ); ?>]" id="fonticon_item_icon_type_<?php echo esc_attr( $item_id );?>" type="radio" value="fonticon"><label for="fonticon_item_icon_type_<?php echo esc_attr( $item_id );?>"><?php esc_html_e('Font icon' , 'kuteshop') ?></label>
            <input <?php checked( 'image', $item_icon_type, true ); ?> name="menu-item-megamenu-item_icon_type[<?php echo esc_attr( $item_id ); ?>]" id="img_item_icon_type_<?php echo esc_attr( $item_id );?>" type="radio" value="image"><label for="img_item_icon_type_<?php echo esc_attr( $item_id );?>"><?php esc_html_e('Image' , 'kuteshop') ?></label>
        </p>
        <p class="field-fonticon" <?php if( $item_icon_type == 'fonticon'):?> style="display:block;"<?php endif;?>>
            ...
            <a data-id="<?php echo esc_attr( $item_id ); ?>" class="button-select-icon" href="#"><?php esc_html_e( 'Select Icon', 'kuteshop' );?></a>
        ...
        <p class="group-image description description-wide" <?php if( $item_icon_type == 'image'):?> style="display:block;"<?php endif;?>>
            ...
            <span class="image-field">
                ...
                <span class="image_icon" style="<?php if( $preview ){ echo "display: inline-block;";} ?>">
                    <img class="preview" src="<?php echo esc_url( $img_preview ); ?>" alt="<?php esc_html_e( 'Image preview', 'kuteshop' ) ?>" title="<?php esc_html_e( 'Image preview', 'kuteshop' ) ?>" />
                </span>
                <a class="kuteshop_image_menu icon_image" data-item_id="<?php echo esc_attr( $item_id );?>" href="#"><?php esc_html_e('Icon Image ','kuteshop');?></a>
            </span>

        </p>
        <?php  if( 'megamenu' == $item->object): ?>
            <p class="mega-menu-setting">
                <label class="row_label" ><strong><?php esc_html_e('Mega menu settings','kuteshop');?></strong></label>
                <br />
