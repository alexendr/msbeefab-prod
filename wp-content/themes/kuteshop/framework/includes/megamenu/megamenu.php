<?php
if ( !class_exists( 'Kuteshop_MegaMenu' ) ) {
	class Kuteshop_MegaMenu
	{
		...
		function __construct()
		{
			$this->custom_fields = array(
				'img_icon', 'do_shortcode',
				'font_icon', 'item_icon_type',
				'mega_menu_width', 'mega_menu_url',
				'img_icon_hover', 'img_note',
				'enable_login_logout', 'enable_minicart',
				'enable_currency_switcher', 'hiden_title',
			);
		...
		function register_scripts( $hook )
		{
			...
			wp_localize_script( 'kuteshop-megamenu', 'kuteshop_ajax_backend', array(
					'ajaxurl'  => admin_url( 'admin-ajax.php' ),
					'security' => wp_create_nonce( 'kuteshop_ajax_backend' ),
				)
			);
		}
            ...
            <div class="icons-popup">
                <div class="inner">
					<?php if ( count( $this->lits_fonts ) > 0 ): ?>
						...
						<?php endforeach; ?>
					<?php else: ?>
						<?php esc_html_e( 'No Font', 'kuteshop' ); ?>
					<?php endif; ?>
                </div>
            </div>
			<?php
		...
		public function addShortcodesCustomCss_megamenu()
		{
			...
			if ( $posts_array ) {
				...
				foreach ( $posts_array as $post ) {
					...
					$shortcodes_custom_css .= get_post_meta( $post->ID, '_Kuteshop_Shortcode_custom_css', true );
				}
				if ( !empty( $shortcodes_custom_css ) ) {
					wp_add_inline_style( 'kuteshop_custom_css', $shortcodes_custom_css );
				}
			}
		}
	}

	new Kuteshop_MegaMenu();
}