...
class Noo_Umbra_Mega_Menu {

	function __construct() {

		// -- Load enqueue script Noo Mega Menu
			add_action( 'admin_enqueue_scripts', array( $this, 'noo_umbra_mega_menu_enqueue_script_admin' ) );
			add_action( 'wp_enqueue_scripts', array( $this, 'noo_umbra_mega_menu_enqueue_script' ) );

		// -- Edit menu walker
			add_filter( 'wp_edit_nav_menu_walker', array( $this, 'noo_umbra_mega_menu_edit_walker'), 10, 2 );

		// -- Setup nav menu item custom fields
			add_filter( 'wp_setup_nav_menu_item', array( $this, 'noo_umbra_mega_menu_set_item' ) );

		// -- Save info custom fields nav menu item
			add_action( 'wp_update_nav_menu_item', array( $this, 'noo_umbra_mega_menu_save_fields'), 10, 3 );
	 ...
	 * @access public
	 * @since  1.0
	 * @name noo_umbra_mega_menu_edit_walker
	 */
	public function noo_umbra_mega_menu_edit_walker( $walker, $menu_id ) {

		return 'Noo_Umbra_Walker_Edit';
	 ...
	 * @access public
	 * @since  1.0
	 * @name noo_umbra_mega_menu_enqueue_script
	 */
	public function noo_umbra_mega_menu_enqueue_script_admin( $hook ) {
	...
	public function noo_umbra_mega_menu_enqueue_script() {
	 ...
	 * @since 	1.0
	 */
	public function noo_umbra_mega_menu_save_fields( $menu_id, $menu_item_db_id, $args ) {
	 ...
	 * @since       1.0 
	*/
	public function noo_umbra_mega_menu_set_item( $menu_item ) {
		// -- set item mega menu
...
new Noo_Umbra_Mega_Menu();

function noo_umbra_notice_set_menu(){
	echo '';
}
