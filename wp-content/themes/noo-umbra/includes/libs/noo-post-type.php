...
if( !function_exists('noo_umbra_registered_post_type') ) :
	function noo_umbra_registered_post_type( $post_type = '', $args = array() ) {
		global $noo_post_types;
		if( empty( $noo_post_types ) ) $noo_post_types = array();

		if( in_array($post_type, array('post','page','attachment','revision','nav_menu_item','product','product_variation','shop_order','shop_order_refund','shop_coupon','shop_webhook','wpcf7_contact_form', 'mc4wp-form', 'tm_global_cp', 'tm_product_cp') ) ) {
			return;
		}
	...
	add_action( 'registered_post_type', 'noo_umbra_registered_post_type', 10, 2 );
endif;

if( !function_exists('noo_umbra_registered_taxonomy') ) :
	function noo_umbra_registered_taxonomy( $taxonomy = '', $object_type = '', $args = array() ) {
		global $noo_post_types;
		if( empty( $noo_post_types ) ) $noo_post_types = array();
	...
	add_action( 'registered_taxonomy', 'noo_umbra_registered_taxonomy', 10, 3 );
endif;

if( !function_exists( 'noo_umbra_get_post_meta' ) ) {
	// Normal get option
	function noo_umbra_get_post_meta( $post_ID = null, $meta_key, $default = null ) {
		...
		return apply_filters( 'noo_umbra_post_meta', $value, $post_ID, $meta_key, $default );
	}
}

if( !function_exists( 'noo_umbra_is_archive' ) ) {
	function noo_umbra_is_archive( $post_type = '' ) {
...
if( !function_exists('noo_umbra_get_term_meta') ) :
	function noo_umbra_get_term_meta( $term_id = null, $meta_key = '', $default = null ) {
		...
		return apply_filters( 'noo_umbra_term_meta', $value, $term_id, $meta_key, $default );
	}
endif;