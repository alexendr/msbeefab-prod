 ...
 *
 * WooCommerce_All_in_One_Currency_Converter_Callbacks class is a class for handling callbacks/ajax.
 *
...
class WooCommerce_All_in_One_Currency_Converter_Callbacks {
	...
	public function __construct($settings){ // $settings = instance of WooCommerce_All_in_One_Currency_Converter_Settings
		...
		// Add WP hooks:		
		if (is_admin()){
			add_action('wp_ajax_wcaiocc_ajax', array($this, 'ajax_request'));
			add_action('wp_ajax_nopriv_wcaiocc_ajax', array($this, 'ajax_request'));
		}
	...
	public function ajax_request(){
		...
		check_ajax_referer('woocommerce-all-in-one-currency-converter-nonce', 'verification', true); // die if nonce incorrect
	...
	public function execute_action($action_data, $cron_execution = false){
		...
		if (empty($action_data)){
			...
			if ($this->settings->restore_defaults() == true){ // let's run restore_defaults method in settings class
				...
				$action_result = __("Default settings have been successfully restored.", 'woocommerce-all-in-one-currency-converter');
			...
			if ($cron_execution == false && !current_user_can('edit_shop_orders')){ // check permissions first (but only for ajax call, not cron)
				exit;
			}
