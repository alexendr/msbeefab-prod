 ...
 *
 * WooCommerce_All_in_One_Currency_Converter_Settings class is a class responsible for retrieving and 
...
class WooCommerce_All_in_One_Currency_Converter_Settings {
	...
	// Default plugin data:
	const PREFIX = 'wcaiocc_';
	const NAME = 'WooCommerce All in One Currency Converter';
	...
	public function get_startup_currency(){
		...
		if (empty($startup_currency)){
			$startup_currency = $this->get_base_currency();
		}
	...
	public function add_cron_job(){
		...
		if (!wp_next_scheduled('wcaiocc_cron_currency_exchange_rate_update_event')){ // add only if not scheduled before
			wp_schedule_event(current_time('timestamp'), 'daily', 'wcaiocc_cron_currency_exchange_rate_update_event');		
			return true;
		} else {
			return false;
		}
	...
	public function remove_cron_job(){
		...
		wp_clear_scheduled_hook('wcaiocc_cron_currency_exchange_rate_update_event');
		return true;
	...
	public function verify_activation_code($code){
		...
		$item = 'WooCommerce All in One Currency Converter';		
	...
	public function restore_defaults(){
		...
		update_option(self::PREFIX . 'remember_user_chosen_currency', 'yes');		
		update_option(self::PREFIX . 'geolocation', 'no');
		...
		update_option(self::PREFIX . 'conversion_method', 'reference');
		...
		update_option(self::PREFIX . 'checkout_total_payment_text', __('Total payment:', 'woocommerce-all-in-one-currency-converter'));
		update_option(self::PREFIX . 'currency_switcher_text', __('Choose your preferred currency', 'woocommerce-all-in-one-currency-converter'));
	...
	public function update_exchange_rates($currencies, $cron_execution = false){
		...
		if ($cron_execution == true){
			...
			$notification_email = get_option(self::PREFIX . 'email_address'); // to email specified in WCAIOCC settings
			if (!empty($failed_currencies) && !empty($notification_email)){ // if some currencies have failed AND user specified an email address in settings
				...
				$subject = __('Currency exchange rate update failed', 'woocommerce-all-in-one-currency-converter');
				$content = __('Error occured while updating your currencies exchange rates. The following currencies were not updated:', 'woocommerce-all-in-one-currency-converter');
				foreach ($failed_currencies as $failed_currency){
					$content .= ' ' . $failed_currency . ',';
				}
				$content = rtrim($content, ','); // remove trailing comma
				$content .= __('. Please go to WooCommerce All in One Currency Converter settings and try chaging the API for listed currencies.', 'woocommerce-all-in-one-currency-converter');
				$headers = 'From: [WordPress] Currency Converter <' . $from_email . '>' . "\r\n";
	...
	private function get_currency_exchange_rate($from, $to, $api){
		...
		$return_data['value'] = apply_filters('wcaiocc_' . strtolower($to) . '_exchange_rate_update_value', $return_data['value']);
