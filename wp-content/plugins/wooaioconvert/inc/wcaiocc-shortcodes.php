...
function wcaiocc_switcher($shortcode_atts = null, $shortcode_content = null){ // parameters only for shortcode
	...
	if (empty($woocommerce_all_in_one_currency_converter->settings->session_currency)){ // if no currency stored in session
		$currency = $default_currency;
	} else {
		$currency = $woocommerce_all_in_one_currency_converter->settings->session_currency;
	}
	
	$currency_data = $woocommerce_all_in_one_currency_converter->settings->get_currency_data(); // get all data on all currencies
	$currency_switcher_text = $woocommerce_all_in_one_currency_converter->settings->get_switcher_data('currency_switcher_text'); // get text above currency switcher
	if (function_exists('icl_register_string')){ // register for WPML
		icl_register_string('woocommerce-all-in-one-currency-converter', 'Currency switcher text', $currency_switcher_text);
	}
	$currency_switcher_theme = $woocommerce_all_in_one_currency_converter->settings->get_switcher_data('currency_switcher_theme'); // get switcher theme
	$currency_switcher_display_template = $woocommerce_all_in_one_currency_converter->settings->get_option('currency_switcher_display_template'); // get currency template
	...
	$switcher_filter_name = 'wcaiocc_currency_switcher_theme_' . $currency_switcher_theme;
	...
	$currency_switcher_html = apply_filters( // last chance to filter the output (regardless of chosen theme)
		'wcaiocc_currency_switcher_html',
...
function wcaiocc_convert($shortcode_atts, $shortcode_content = null) { // parameters for shortcode
	...
	if (!empty($_POST['wcaiocc_change_currency_code'])) { // user have manually switched the currency
		$active_currency = $_POST['wcaiocc_change_currency_code'];
	} else if (!empty($_COOKIE['wcaiocc_user_currency_session'])){
		$active_currency = $_COOKIE['wcaiocc_user_currency_session'];
	} else if (!empty($_COOKIE['wcaiocc_user_currency_cookie']) && $this->settings->remember_user_currency()){ // user currency from cookie (remember)
		$active_currency = $_COOKIE['wcaiocc_user_currency_cookie'];
	} else if ('yes' == get_option('wcaiocc_geolocation')){ // if IP geolocation enabled
		...
		$country_list = get_option('wcaiocc_country_list_data');
	...
	if ($from_currency != $base_currency){ // if not base currency - get exchange rate
		$from_exchange_rate = get_option('wcaiocc_exchange_rate_' . $from_currency);
	}

	if ($to_currency != $base_currency){ // if not base currency - get exchange rate
		$to_exchange_rate = get_option('wcaiocc_exchange_rate_' . $to_currency);
	}
	...
	if ($format_price == true){ // we only need currency data if we are formatting the price
		...
		if ($to_currency != $base_currency){ // if target currency is NOT base currency, we need to get its data from plugin settings
			$available_currencies_json = get_option('wcaiocc_available_currencies'); // here we have all currencies data EXCEPT exchange rate
