 ...
 *
 * WooCommerce_All_in_One_Currency_Converter_Main class is a plugin main wrapper class.
 *
...
class WooCommerce_All_in_One_Currency_Converter_Main {
	...
	public function __construct($plugin_path){
		...
		$this->settings = new WooCommerce_All_in_One_Currency_Converter_Settings($plugin_path); 
		
		// run callbacks-handling class:		
		$this->callbacks = new WooCommerce_All_in_One_Currency_Converter_Callbacks($this->settings); // run callbacks class constructor and pass settings object to it
		...
		if ('automatic' == $this->settings->get_exchange_rates_update_method()){
			...
			add_action('wcaiocc_cron_currency_exchange_rate_update_event', 'wcaiocc_cron_currency_exchange_rate_update');
		} else { // unschedule
		...
		add_shortcode('wcaiocc_switcher', 'wcaiocc_switcher'); // register switcher shortcode
		add_shortcode('wcaiocc_convert', 'wcaiocc_convert'); // register conversion shortcode
		...
		switch ($this->mode){
			case 'admin':
				new WooCommerce_All_in_One_Currency_Converter_Admin($this->settings); // run admin mode class constructor and pass settings object
				break;
			case 'frontend':
				new WooCommerce_All_in_One_Currency_Converter_Frontend($this->settings); // run frontend mode class constructor and pass settings object		
				break;
			default :
				return false;
		}				
	...
	public function add_widget(){
		...
		register_widget('WooCommerce_All_in_One_Currency_Converter_Widget');
	...
	public function plugin_activate(){
		...
		if (!empty($flag)){
			...
			wp_die(
				sprintf(__('<p><strong>%s</strong> plugin requires %s version %s or greater.</p>', 'woocommerce-all-in-one-currency-converter'), 
					'WooCommerce All in One Currency Converter', $flag, $version
	...
	public function load_plugin_textdomain(){
		...
		$dir_name = basename($dir_name); // this gives us only last directory in path (plugin directory - woocommerce-all-in-one-currency-converter)
		$dir_name .= '/languages'; // this gives us 'woocommerce-all-in-one-currency-converter/languages'
		load_plugin_textdomain('woocommerce-all-in-one-currency-converter', false, $dir_name);
