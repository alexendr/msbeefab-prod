 ...
 *
 * WooCommerce_All_in_One_Currency_Converter_Frontend class is a frontend-class for the plugin.
 *
...
class WooCommerce_All_in_One_Currency_Converter_Frontend {
	...
	private $conversion_method = 'reference'; // reference or checkout
	...
	public function __construct($settings){ // $settings = instance of WooCommerce_All_in_One_Currency_Converter_Settings
		...
		if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))){
			...
			if ($this->conversion_method == 'reference'){ // conversion only for user reference
				...
				// Shipping:
				add_filter('woocommerce_package_rates', array($this, 'custom_shipping_price'), 9999, 2); // convert shipping costs
	...
	public function init_session() {
		...
		if (!empty($_POST['wcaiocc_change_currency_code'])) { // user have manually switched the currency

			$new_currency = $_POST['wcaiocc_change_currency_code'];
			if ($this->settings->remember_user_currency()) { // if user currency should be remembered
				wc_setcookie('wcaiocc_user_currency_cookie', $new_currency, time() + 7776000); // store it in a cookie for 90 days
			}
		...
		} else if (!empty($_COOKIE['wcaiocc_user_currency_session'])){ // currency already in transient cookie
			
			$new_currency = $_COOKIE['wcaiocc_user_currency_session']; 
		
		} else if ($this->settings->remember_user_currency() && !empty($_COOKIE['wcaiocc_user_currency_cookie'])){ // user currency from cookie (remember)
		
			$new_currency = $_COOKIE['wcaiocc_user_currency_cookie'];
			...
			$country_code = apply_filters('wcaiocc_visitor_country_code', $location_info['country']);
		...
		if (empty($_COOKIE['wcaiocc_user_currency_session']) || $_COOKIE['wcaiocc_user_currency_session'] != $new_currency){ // if transient cookie empty or different from new currency			
			wc_setcookie('wcaiocc_user_currency_session', $new_currency, 0); // save transient cookie
			$this->currency_changed = true;
		}
	...
	public function add_scripts(){
		...
		if (defined('WCAIOCC_DEBUG')){
			wp_enqueue_style('wcaiocc-frontend-style-handle', $this->settings->get_plugin_dir_url() . 'css/wcaiocc-frontend.css');		
		} else {
			wp_enqueue_style('wcaiocc-frontend-style-handle', $this->settings->get_plugin_dir_url() . 'css/wcaiocc-frontend.min.css');	
		}
		
		wp_add_inline_style('wcaiocc-frontend-style-handle', $this->settings->get_switcher_data('currency_switcher_css')); // add user custom css rules

		// Add main frontend JS file (make sure jquery is enqueued as well):
		if (defined('WCAIOCC_DEBUG')){
			wp_enqueue_script('wcaiocc-frontend-script-handle', $this->settings->get_plugin_dir_url() . 'js/wcaiocc-frontend.js', array('jquery'));
		} else {
			wp_enqueue_script('wcaiocc-frontend-script-handle', $this->settings->get_plugin_dir_url() . 'js/wcaiocc-frontend.min.js', array('jquery'));
		}
		...
		$js_variables = array(
			...
			'currency_change_key' => 'wcaiocc_change_currency_code', // $_POST array key used to change currency by user
		...
		wp_localize_script('wcaiocc-frontend-script-handle', 'wcaiocc_vars_data', $js_variables);
	...
	public function show_total_in_base_currency($html = ''){
		...
		if ($this->default_currency == true){ // return input parameter if we are using default currency
			...
			$price_template = apply_filters('wcaiocc_checkout_total_in_base_currency_html', '<br /><span class="wcaiocc_checkout_total_in_base_currency">%s %s</span>');
			
			$total_text = $this->settings->get_option('checkout_total_payment_text');
			if (empty($total_text)){
				$total_text = __('Total payment:', 'woocommerce-all-in-one-currency-converter');
			}
	...
	public function custom_coupon_discount($is_coupon_custom, $coupon_code){
		...
		if ($coupon_data['discount_type'] != 'percent_product' && $coupon_data['discount_type'] != 'percent'){
			$amount = $this->convert_price($amount);
		}
