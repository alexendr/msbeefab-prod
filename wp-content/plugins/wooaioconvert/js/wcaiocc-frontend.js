...
/* ---------------------------- SWITCHER THEMES ---------------------------- */
	...
	jQuery(document).ready(function ($) {
		...
		$(document).on('change', 'select.wcaiocc-select', function () { // submit post data on dropdown change
			$('#wcaiocc-currency-input').val($(this).val()); // assign value from dropdown list to invisible input field
			$('#wcaiocc-currency-form').submit(); // and submit it
		});
	...
	jQuery(document).ready(function ($) {
		...
		$(document).on('click', 'a.wcaiocc-list-item-link', function () { // submit post data on link click
			if ($(this).hasClass('selected')) { // if currency is already chosen
				return false; // do nothing
			} else {
				$('#wcaiocc-currency-input').val($(this).attr('data-wcaiocc-currency')); // assign data attribute from clicked link to invisible input field
				$('#wcaiocc-currency-form').submit(); // and submit it
			}
		});
...
/* ---------------------------- INVISIBLE FORM ---------------------------- */
	...
	jQuery(document).ready(function ($) {
		...
		$('body').append('<form method="post" id="wcaiocc-currency-form" style="display:none"><input id="wcaiocc-currency-input" type="text" name="' + wcaiocc_vars_data.currency_change_key + '" /></form>');
...
/* ---------------------------- PRICE SLIDER COMPATIBILITY ---------------------------- */
	...
	jQuery(document).ready(function ($) {
		...
		if (typeof woocommerce_price_slider_params !== 'undefined' && // price slider is active
			wcaiocc_vars_data.js_price_slider == true) { // and we are running WC 2.6 or higher (remove this condition in later versions)

			woocommerce_price_slider_params.currency_symbol = wcaiocc_vars_data.currency_data.symbol;
			woocommerce_price_slider_params.currency_pos = wcaiocc_vars_data.currency_data.position;
			...
			$(document.body).bind('price_slider_updated', function(event, min, max) { // convert the displayed prices every time the slider is created/updated
				...
				var from_value = Math.floor(min * wcaiocc_vars_data.currency_data.rate); // round down
				var to_value = Math.ceil(max * wcaiocc_vars_data.currency_data.rate); // round up
...
/* ---------------------------- WC COMPOSITE PRODUCTS COMPATIBILITY ---------------------------- */
	...
	if (wcaiocc_vars_data.conversion_method == 'reference' && // reference conversion method and...
		wcaiocc_vars_data.base_currency != wcaiocc_vars_data.currency && // active currency different than base currency and...
		wcaiocc_vars_data.currency_data['rate'] != 1){ // exchange rate different than 1) 
				...
				$(function () {					
					...
					update_totals = function () { // this function is called every time there is any change made to the observerNode
						...
						$('.cart.composite_data').each(function () {
							...
							var price_widget_selector = '.widget_composite_summary_price'; // this will only be used to update the optional widget total price
							var price_html = '';
							...
							var symbol = wcaiocc_vars_data.currency_data.symbol;
							var rate = wcaiocc_vars_data.currency_data.rate;
							var position = wcaiocc_vars_data.currency_data.position;
							var number_decimals = wcaiocc_vars_data.currency_data.number_decimals;
							var decimal_separator = wcaiocc_vars_data.currency_data.decimal_separator;
							var thousand_separator = wcaiocc_vars_data.currency_data.thousand_separator;
...
// Make sure currency is switched in AJAX cart after user has changed it
	...
	jQuery(document).ready(function ($) {
		...
		if (wcaiocc_vars_data.currency_changed == 1 && typeof wc_cart_fragments_params !== 'undefined') { // if currency has been changed AND wc_cart_fragments_params is defined
...
// Hugely inspired by WooCommerce ajax geolocation (geolocation.js)
	...
	jQuery(function ($) {
		...
		if ($(wcaiocc_vars_data.page_cache_support_data).length > 0) { // first check if we are using the page cache support

			var page_cache_data = wcaiocc_vars_data.page_cache_support_data;
			...
			if (
				...
				var post_data = {
					action: 'wcaiocc_ajax',
					verification: wcaiocc_vars_data.wp_nonce,
					execute: 'get_currency_hash'
				};
				// Send Ajax request:
				jQuery.post(wcaiocc_vars_data.ajaxurl, post_data, function (response) {
