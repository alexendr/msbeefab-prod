...
function noo_create_meta_box( $post, $meta_box ) {
	...
	foreach ( $meta_box['fields'] as $field ) {
		...
		$id  = $field['id'];
		$meta = noo_umbra_get_post_meta( $post->ID, $id );
...
function noo_render_metabox_fields ( $post, $id, $type, $meta, $std, $field = null ) {
	switch( $type ) {
		...
		case 'gallery':
			...
			$btn_text = !empty( $meta ) ? esc_html__( 'Edit Gallery', 'noo-umbra-core' ) : esc_html__( 'Add Images', 'noo-umbra-core' );
			...
			echo '<input type="button" class="button" name="' . $id . '_button_clear" id="' . $id . '_clear" value="' . esc_html__( 'Clear Gallery', 'noo-umbra-core' ) . '" />';
...
?>
				...
				jQuery(document).ready(function($) {
					...
					$('#<?php echo esc_attr($id); ?>_upload').on('click', function(event) {
						...
						wp_media_frame.on('update', function(){
							...
							noo_upload_btn.attr('value', '<?php echo esc_html__( 'Edit Gallery', 'noo-umbra-core' ); ?>');
					...
					$('#<?php echo esc_attr($id); ?>_clear').on('click', function(event) {
						...
						noo_clear_btn.hide();
						$('#<?php echo esc_attr($id); ?>_upload').attr('value', '<?php echo esc_html__( 'Add Images', 'noo-umbra-core' ); ?>');
		...
		case 'media':
			...
			$val = $meta ? $meta : $std;
			$btn_text = !empty( $val ) ? esc_html__( 'Change File', 'noo-umbra-core' ) : esc_html__( 'Select File', 'noo-umbra-core' );
			echo '<input type="text" name="noo_meta_boxes[' . $id . ']" id="' . $id . '" value="' . ( $meta ? $meta : $std ) . '" style="margin-bottom:10px" />';
			echo '<input type="button" class="button button-primary" name="' . $id . '_button_upload" id="' . $id . '_upload" value="' . $btn_text . '" />';
			echo '<input type="button" class="button" name="' . $id . '_button_clear" id="' . $id . '_clear" value="' . esc_html__( 'Clear File', 'noo-umbra-core' ) . '" />';
			?>
			<script>
				jQuery(document).ready(function($) {
					...
					$('#<?php echo esc_attr($id); ?>_upload').on('click', function(event) {
						...
						var wp_media_frame = wp.media.frames.wp_media_frame = wp.media({
							title: "<?php echo esc_html__( 'Select or Upload your File', 'noo-umbra-core' ); ?>",
							button: {
								text: "<?php echo esc_html__( 'Select', 'noo-umbra-core' ); ?>"
							},
							<?php if($type == 'media'):?>
						...
						wp_media_frame.on('select', function(){
							...
							noo_upload_btn.attr('value', '<?php echo esc_html__( 'Change File', 'noo-umbra-core' ); ?>');
							$('#<?php echo esc_attr($id); ?>_clear').css('display', 'inline-block');
						});
					...
					$('#<?php echo esc_attr($id); ?>_clear').on('click', function(event) {
						var noo_clear_btn = $(this);
						noo_clear_btn.hide();
						$('#<?php echo esc_attr($id); ?>_upload').attr('value', '<?php echo esc_html__( 'Select File', 'noo-umbra-core' ); ?>');
						noo_clear_btn.siblings('#<?php echo esc_attr($id); ?>').val('');
					});
				});
			</script>
		...
		case 'image':
			...
			$btn_text = !empty( $image_id ) ? esc_html__( 'Change Image', 'noo-umbra-core' ) : esc_html__( 'Select Image', 'noo-umbra-core' );
			echo '<input type="hidden" name="noo_meta_boxes[' . $id . ']" id="' . $id . '" value="' . ( $meta ? $meta : $std ) . '" />';
			echo '<input type="button" class="button button-primary" name="' . $id . '_button_upload" id="' . $id . '_upload" value="' . $btn_text . '" />';
			echo '<input type="button" class="button" name="' . $id . '_button_clear" id="' . $id . '_clear" value="' . esc_html__( 'Clear Image', 'noo-umbra-core' ) . '" />';
			echo '<div class="noo-thumb-wrapper">' . $output . '</div>';
?>
			<script>
				jQuery(document).ready(function($) {
					...
					$('#<?php echo esc_attr($id); ?>_upload').on('click', function(event) {
						...
						// I decided to create new frame every time to control the selected images
						var wp_media_frame = wp.media.frames.wp_media_frame = wp.media({
							title: "<?php echo esc_html__( 'Select or Upload your Image', 'noo-umbra-core' ); ?>",
							button: {
								text: "<?php echo esc_html__( 'Select', 'noo-umbra-core' ); ?>"
							},
							library: { type: 'image' },
						...
						wp_media_frame.on('select', function(){
							...
							noo_upload_btn.attr('value', '<?php echo esc_html__( 'Change Image', 'noo-umbra-core' ); ?>');
							$('#<?php echo esc_attr($id); ?>_clear').css('display', 'inline-block');
						});
					...
					$('#<?php echo esc_attr($id); ?>_clear').on('click', function(event) {
						var noo_clear_btn = $(this);
						noo_clear_btn.hide();
						$('#<?php echo esc_attr($id); ?>_upload').attr('value', '<?php echo esc_html__( 'Select Image', 'noo-umbra-core' ); ?>');
						noo_clear_btn.siblings('#<?php echo esc_attr($id); ?>').val('');
						noo_clear_btn.siblings('.noo-thumb-wrapper').html('');
					});
				});
...
?>
		...
		case 'page_layout':
			...
			switch( $post_layout ) {
				case 'fullwidth':
					$post_layout_text = esc_html__( 'Full-Width Page', 'noo-umbra-core' );
					break;
				case 'sidebar':
					$post_layout_text = esc_html__( 'Page With Right Sidebar', 'noo-umbra-core' );
					break;
				case 'left_sidebar':
					$post_layout_text = esc_html__( 'Page With Left Sidebar', 'noo-umbra-core' );
					break;
			}
			
			echo '<p>' . sprintf( noo_umbra_kses( __( 'Global setting for the Layout of Single Post page is: <strong>%s</strong>', 'noo-umbra-core') ), $post_layout_text ) . '</p>';
			if ( $post_layout != 'fullwidth' ) {
				$sidebar_text = noo_umbra_get_sidebar_name( $sidebar );
				echo '<p>' . sprintf( noo_umbra_kses( __( 'And the Sidebar is: <strong>%s</strong>', 'noo-umbra-core') ), $sidebar_text ) . '</p>';
			}
		...
		case 'menus':
			...
			echo'<select name="noo_meta_boxes[' . $id . ']" >';
			echo'	<option value="" '. selected( $meta, '', true ) . '>' . esc_html__( 'Don\'t Need Menu', 'noo-umbra-core') . '</option>';
		...
		case 'users':
			...
			echo'<select name="noo_meta_boxes[' . $id . ']" >';
			echo'	<option value="" '. selected( $meta, '', true ) . '>' . esc_html__( 'No User', 'noo-umbra-core') . '</option>';
		...
		case 'rev_slider':
			...
			echo '<select name="noo_meta_boxes[' . $id . ']">';
			echo '<option value="">' . esc_html__( ' - No Slider - ', 'noo-umbra-core') . '</option>';
