...
if( !function_exists( 'noo_cusomizer_upload_settings' ) ) :
	function noo_cusomizer_upload_settings(){
		...
		if ( $file_ext == 'json' ) {
			...
			$file = noo_umbra_handle_upload_file($file);
...
function noo_ajax_get_menu(){
	...
	<?php if ( has_nav_menu( $menu_location ) ) :
		...
		echo '<ul class="noo-menu"><li><a href="' . home_url( '/' ) . 'wp-admin/nav-menus.php">' . esc_html__( 'Assign a menu', 'noo-umbra-core' ) . '</a></li></ul>';
	endif; ?>
