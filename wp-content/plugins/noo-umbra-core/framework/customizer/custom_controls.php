...
function noo_customizer_add_custom_controls( $wp_customize ) {
	...
	class NOO_Customizer_Control_Checkbox extends WP_Customize_Control {
		...
		public function __construct( $manager, $id, $args = array(), $options = array() ) {
			...
			if ( isset( $args['data_on'] ) && !empty( $args['data_on'] ) ) {
				...
				$this->data_on = esc_html__( 'ON', 'noo-umbra-core' );
			}

			if ( isset( $args['data_off'] ) && !empty( $args['data_off'] ) ) {
				$this->data_off = $args['data_off'];
			} else {
				$this->data_off = esc_html__( 'OFF', 'noo-umbra-core' );
			}
		}
	...
	class NOO_Customize_Image_Control extends WP_Customize_Image_Control {
		...
		public function __construct( $manager, $id, $args = array(), $options = array() ) {
			...
			$this->add_tab( 'library',  esc_html__( 'Media Library', 'noo-umbra-core' ), array( $this, 'media_library' ) );
		...
		public function media_library() {
			?>
				<a class="choose-from-library-link button" data-controller="<?php echo esc_attr($this->id); ?>">
					<?php echo esc_html__( 'Open Library', 'noo-umbra-core' ); ?>
				</a>
			<?php
		}
	...
	class NOO_Customizer_Control_Widgets_Select extends WP_Customize_Control {
      ...
      <label>
        ...
        <select <?php $this->link(); ?>>
          <option value=""><?php echo esc_html__( '- Select Sidebar -', 'noo-umbra-core' ); ?></option>
          <?php
	...
	class NOO_Customizer_Control_Pages_Select extends WP_Customize_Control {
		...
		public function render_content() {
			...
			$dropdown = str_replace( '<select', '<select ' . $this->get_link() . 'class="noo-customize-chosen" data-placeholder="' . esc_html__( '- Select Page -', 'noo-umbra-core' ) . '"', $dropdown );
		?>
      <label>
        <span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
        <?php echo $dropdown; ?>
	...
	class NOO_Customizer_Control_Google_Fonts extends WP_Customize_Control {
		...
		public function __construct( $manager, $id, $args = array(), $options = array() ) {
			...
			$this->weight = empty( $this->weight ) ? noo_umbra_get_theme_default( 'font_weight' ) : $this->weight;
			...
			if ( empty ( self::$font_weight_name ) )
				self::$font_weight_name = array(
					'100'       => esc_html__( 'Extra Light', 'noo-umbra-core' ),
					'100italic' => esc_html__( 'Extra Light Italic', 'noo-umbra-core' ),
					'200'       => esc_html__( 'Light', 'noo-umbra-core' ),
					'200italic' => esc_html__( 'Light Italic', 'noo-umbra-core' ),
					'300'       => esc_html__( 'Book', 'noo-umbra-core' ),
					'300italic' => esc_html__( 'Book Italic', 'noo-umbra-core' ),
					'regular'   => esc_html__( 'Regular', 'noo-umbra-core' ),
					'italic'    => esc_html__( 'Regular Italic', 'noo-umbra-core' ),
					'500'       => esc_html__( 'Medium', 'noo-umbra-core' ),
					'500italic' => esc_html__( 'Medium Italic', 'noo-umbra-core' ),
					'600'       => esc_html__( 'Semi-bold', 'noo-umbra-core' ),
					'600italic' => esc_html__( 'Semi-bold Italic', 'noo-umbra-core' ),
					'700'       => esc_html__( 'Bold', 'noo-umbra-core' ),
					'700italic' => esc_html__( 'Bold Italic', 'noo-umbra-core' ),
					'800'       => esc_html__( 'Extra-bold', 'noo-umbra-core' ),
					'800italic' => esc_html__( 'Extra-bold Italic', 'noo-umbra-core' ),
					'900'       => esc_html__( 'Heavy', 'noo-umbra-core' ),
					'900italic' => esc_html__( 'Heavy Italic', 'noo-umbra-core' ),
				);
		}
		...
		?>
          <span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
          <div class="noo-font-selects xxx">
            <select class="noo-font-family noo-customize-chosen" data-placeholder="<?php echo esc_html__( '- Font Family -', 'noo-umbra-core' ); ?>" <?php $this->link(); ?>>
            <option value=""></option>
              <?php
				$selected_font_id = 0;
		...
		?>
            ...
            <select class="noo-font-weight-and-style">
              <option value=""><?php echo esc_html__( '- Styles -', 'noo-umbra-core' ); ?></option>
              <?php
		...
		?>
            ...
            <select class="noo-font-subset" data-customize-setting-link="<?php echo esc_attr($this->id) . '_subset'; ?>">
              <option value=""><?php echo esc_html__( '- Subsets -', 'noo-umbra-core' ); ?></option>
              <?php
	...
	class NOO_Customizer_Control_MailChimp_Mail_List extends WP_Customize_Control {
		...
		public function render_content() {
			...
			<label>
				<span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
				<input <?php $this->link(); ?> class="regular-text" type="text" placeholder="<?php echo esc_html__( 'Mailchimp API key', 'noo-umbra-core'); ?>" value="<?php echo esc_attr( $this->value() ); ?>"/>
				<p class="description"><?php echo sprintf( noo_umbra_kses( __( 'Enter Mailchimp API key to connect to your account. Don\'t know where to find? <a href="%s" target="_BLANK">Just click here to get your API</a>', 'noo-umbra-core') ), 'https://us5.admin.mailchimp.com/account/api/'); ?></p>
			</label>
