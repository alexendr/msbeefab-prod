...
function noo_theme_option_custom_css() {
	if ( noo_umbra_get_option( 'noo_custom_css', '' ) ) :
?>
    <style id="noo-custom-css" type="text/css"><?php echo noo_umbra_get_option( 'noo_custom_css', '' ); ?></style>
  <?php
	endif;
}
...
function noo_theme_option_custom_js() {
	if ( noo_umbra_get_option( 'noo_custom_javascript', '' ) ) :
?>
	<script>
		<?php echo noo_umbra_get_option( 'noo_custom_javascript', '' ); ?>
	</script>
	<?php
	endif;
}
...
if ( ! function_exists( 'noo_customizer_controls_l10n' ) ) :
	function noo_customizer_controls_l10n() {
		return array(
			'navbar_height' => esc_html__( 'NavBar Height (px)', 'noo-umbra-core'),
			'mobile_navbar_height' => esc_html__( 'Mobile NavBar Height (px)', 'noo-umbra-core'),
			'ajax_update_msg'    => esc_html__( 'Updating ...', 'noo-umbra-core' ),
			'import_error_msg' => esc_html__( 'Error when parsing your file.', 'noo-umbra-core' ),
			'export_preparing_msg' => esc_html__( 'We are preparing your export file, please wait...', 'noo-umbra-core' ),
			'export_fail_msg' => esc_html__( 'There was a problem generating your export file, please try again.', 'noo-umbra-core' ),
...
if ( ! function_exists( 'noo_customizer_live_data' ) ) :
	function noo_customizer_live_data() {
		...
		$customizer_live_data = array(
			...
			'ok'					=> esc_html__( 'Yes', 'noo-umbra-core' ),
			'cancel'				=> esc_html__( 'No', 'noo-umbra-core' ),
			'ajax_update_msg'		=> esc_html__( 'Updating ...', 'noo-umbra-core' ),
			'cannot_preview_msg'	=> esc_html__( 'This option doesn\'t support live preview. Save it and see the change on your site.', 'noo-umbra-core' ),
			'redirect_msg'			=> esc_html__( 'Wanna go to %s to see the change?', 'noo-umbra-core' ),
			'blog_text'				=> esc_html__( 'Blog Page', 'noo-umbra-core' ),
			'shop_text'				=> esc_html__( 'Shop Page', 'noo-umbra-core' ),
			'archive_text'			=> esc_html__( 'An Archive Page', 'noo-umbra-core' ),
			'post_text'				=> esc_html__( 'A Post', 'noo-umbra-core' ),
			'product_text'			=> esc_html__( 'A Product', 'noo-umbra-core' ),
		);
		...
		if( !empty( $noo_post_types ) ) {
			foreach ($noo_post_types as $post_type => $args) {
				...
				$archive_text = sprintf( esc_html__( '%s Archive Page', 'noo-umbra-core'), $args['name'] );
				...
				$single_text	= sprintf( esc_html__( 'A %s Page', 'noo-umbra-core'), $args['singular_name'] );
...
if ( ! function_exists( 'noo_enqueue_customizer_live_js' ) ) :
	function noo_enqueue_customizer_live_js() {
		...
		wp_register_script( 'noo-customizer-live-core-js', NOO_FRAMEWORK_URI . '/assets/js/noo-customizer-live-core.js', array( 'jquery', 'vendor-alertify-js' ), null, true );
...
if ( ! function_exists( 'noo_output_css_file' ) ) :
	function noo_output_css_file($creds) {
		...
		$css_dir = noo_umbra_create_upload_dir( $wp_filesystem );
...
if ( ! function_exists( 'noo_jbst_tmpadminheader' ) ) :
	function noo_jbst_tmpadminheader() {
		...
		$title = esc_html__( 'Customizer credentials', 'noo-umbra-core');
