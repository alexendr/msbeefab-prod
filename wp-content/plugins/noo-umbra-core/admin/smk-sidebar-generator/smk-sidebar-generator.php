...
class SMK_Sidebar_Generator {
	...
	*/
	public function __construct(){
		...
		$this->sbg_name = esc_html__('Sidebar Generator', 'noo-umbra-core');
		...
		if( 'themes.php' == $pagenow && isset( $_GET['page'] ) && $_GET['page'] == strtolower( __CLASS__ ) ){
			...
			wp_enqueue_script('jquery-ui-slider');
	 		...
	 		wp_localize_script('smk_sbg_scripts', 'smk_sbg_lang', array(
					'remove'        => esc_html__('Remove', 'noo-umbra-core'),
					'not_saved_msg' => esc_html__("You've made changes, don't forget to save.", 'noo-umbra-core'),
					'ok'            => esc_html__("Changes were saved successfully.", 'noo-umbra-core'),
					'fail'          => esc_html__("An unexpected error ocurred.", 'noo-umbra-core'),
					'created'       => esc_html__("The sidebar was successfully created.", 'noo-umbra-core'),
					's_exists'      => esc_html__("The sidebar already exists. Please change the name.", 'noo-umbra-core'),
					'empty'         => esc_html__("Please enter a name for this sidebar.", 'noo-umbra-core'),
					's_remove'      => esc_html__("Are you sure? If you remove this sidebar it can't be restored.", 'noo-umbra-core'),
					's_removed'     => esc_html__("Sidebar Removed", 'noo-umbra-core'),
					'data_imported' => esc_html__("Data imported successfully.", 'noo-umbra-core'),
	...
	*/
	...
	$text = array(
			esc_html__('Add new', 'noo-umbra-core'),//0
			esc_html__('Save Changes', 'noo-umbra-core'),//1
			esc_html__('Sidebar Name:', 'noo-umbra-core'),//2
			esc_html__('ID:', 'noo-umbra-core'),//3
			esc_html__('Shortcode:', 'noo-umbra-core'),//4
			esc_html__('Remove', 'noo-umbra-core'),//5
			esc_html__('Import', 'noo-umbra-core'),//6
		);
	...
	echo '<div class="wrap smk_sbg_main_block">';
		...
		echo '<div class="sbg_clearfix">';
			...
			echo '<div class="sbg_grid_50">';
				...
				echo '<div class="smk_sbg_main_menu">
						<span data-id="tab_main_form" class="active">'. esc_html__('Sidebars', 'noo-umbra-core') .'</span>
						<span data-id="tab_export">'. esc_html__('Export', 'noo-umbra-core') .'</span>
						<span data-id="tab_import">'. esc_html__('Import', 'noo-umbra-core') .'</span>
						<span data-id="tab_how_to">'. esc_html__('How to use?', 'noo-umbra-core') .'</span>
				</div>';
			echo '</div>';
		echo '</div>';
		...
		echo '<div id="tab_export" class="smk_sbg_tab additional">';
			//Export form
			echo '<div class="smk_sbg_label">' . esc_html__('Copy text from textarea:','noo-umbra-core') .'</div>';
		...
		echo '<div id="tab_import" class="smk_sbg_tab additional">';
			//Import form
			echo '<div class="smk_sbg_label">' . esc_html__('Paste exported data in textarea:','noo-umbra-core') .'</div>';
		...
		echo '<div id="tab_how_to" class="smk_sbg_tab additional">';
			//Import form
			echo '<h2>' . esc_html__('How to use?','noo-umbra-core') .'</h2>';
			echo '<h3>' . esc_html__('Shortcode:','noo-umbra-core') .'</h3>';
			echo '<p>' . esc_html__('Paste the shortcode anywhere you want, in posts, pages etc. If the input accept shortcodes, then the sidebar will be displayed.','noo-umbra-core') .'</p>';
			echo '<pre>[smk_sidebar id="SIDEBAR_ID"]</pre>';
			echo '<h3>' . esc_html__('Function:','noo-umbra-core') .'</h3>';
			echo '<p>' . esc_html__('You can use the built-in function, but for this you should modify theme files and make sure to check if the function exists before use it.','noo-umbra-core') .'</p>';
			echo '<pre>
if(function_exists("smk_sidebar"){
 	smk_sidebar("SIDEBAR_ID");
}
			</pre>';
			echo '<h3>' . esc_html__('WP Native function:','noo-umbra-core') .'</h3>';
			echo '<p>' . esc_html__('You can use the built-in function <em>smk_sidebar</em>, but anyways I recommend using WP native function to avoid conflicts.','noo-umbra-core') .'</p>';
			echo "<pre>
if(function_exists('dynamic_sidebar') && dynamic_sidebar('SIDEBAR_ID')) :
endif;
			</pre>";

			echo '<h3>' . esc_html__('For more info visit the following links:','noo-umbra-core') .'</h3>';
			...
			if( isset($exported_data) && !empty($exported_data) ){
				if(is_serialized(base64_decode($exported_data))){
					...
					if( is_array($exported_data) ){
						...
						echo esc_html__('Data is not valid.','noo-umbra-core');
					}
				}
				else{
					echo esc_html__('Data is not valid.','noo-umbra-core');
				...
				echo esc_html__('Please enter data.','noo-umbra-core');
			}
