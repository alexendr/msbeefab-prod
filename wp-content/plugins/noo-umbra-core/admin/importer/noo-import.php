...
class Noo_Import_Demo extends WP_Importer{
	...
	public function process_data() {
		// --- Check security request
			...
			set_time_limit(0);
		// --- load file demo
			$file_demo = NOO_PLUGIN_SERVER_PATH . "/admin/importer/data-demo/{$_POST['name']}/content.xml";
			$widget = NOO_PLUGIN_SERVER_PATH . "/admin/importer/data-demo/{$_POST['name']}/widgets.wie";
			$option = NOO_PLUGIN_SERVER_PATH . "/admin/importer/data-demo/{$_POST['name']}/option.json";


			$this->import( $file_demo );
			$this->_noo_process_import_file( $widget );
			$this->process_option( $option );
			echo esc_html__( 'Import Successfully!', 'noo-umbra-core' );
			...
			if ( ! empty( $_POST['user_map'][$i] ) ) {
				...
				if ( ! is_wp_error( $user_id ) ) {
					...
					if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG ) {
						printf( esc_html__( 'Failed to create new user for %s. Their posts will be attributed to the current user.', 'noo-umbra-core' ), esc_html($this->authors[$old_login]['author_display_name']) );
	...
	function process_posts() {
		...
		foreach ( $this->posts as $post ) {
			...
			if ( ! post_type_exists( $post['post_type'] ) ) {
				printf( esc_html__( 'Failed to import &#8220;%s&#8221;: Invalid post type %s', 'noo-umbra-core' ),
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				// printf( esc_html__( '%s &#8220;%s&#8221; already exists.', 'noo-umbra-core'), $post_type_object->labels->singular_name, esc_html($post['post_title']) );
				// echo '<br />';
				...
				if ( is_wp_error( $post_id ) ) {
					if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG ) {
						printf( esc_html__( 'Failed to import %s &#8220;%s&#8221;', 'noo-umbra-core' ),
				...
				foreach ( $post['terms'] as $term ) {
					...
					if ( ! $term_id ) {
						...
						if ( ! is_wp_error( $t ) ) {
							...
							if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG ) {
								printf( esc_html__( 'Failed to import %s %s', 'noo-umbra-core' ), esc_html($taxonomy), esc_html($term['name']) );
	...
	function noo_update_nav_menu_item( $menu_id = 0, $menu_item_db_id = 0, $menu_item_data = array() ) {
	    ...
	    if ( ! empty( $menu_item_db_id ) && ! is_nav_menu_item( $menu_item_db_id ) )
	        return new WP_Error( 'update_nav_menu_item_failed', esc_html__( 'The given object ID is not that of a menu item.', 'noo-umbra-core' ) );
	 
	    $menu = wp_get_nav_menu_object( $menu_id );
	 
	    if ( ! $menu && 0 !== $menu_id ) {
	        return new WP_Error( 'invalid_menu_id', esc_html__( 'Invalid menu ID.', 'noo-umbra-core' ) );
	    }
	...
	function fetch_remote_file( $url, $post ) {
		...
		$headers = wp_remote_get( $url, $upload['file'] );
	...
	function header() {
		echo '<div class="wrap">';
		echo '<h2>' . esc_html__( 'Import WordPress', 'noo-umbra-core' ) . '</h2>';
		...
		if ( isset( $updates[$basename] ) ) {
			...
			echo '<div class="error"><p><strong>';
			printf( esc_html__( 'A new version of this importer is available. Please update to version %s to ensure compatibility with newer export files.', 'noo-umbra-core' ), $update->update->new_version );
			echo '</strong></p></div>';
		}
	}
	...
	function greet() {
		...
		echo '<p>' . esc_html__( 'Howdy! Upload your WordPress eXtended RSS (NOO) file and we&#8217;ll import the posts, pages, comments, custom fields, categories, and tags into this site.', 'noo-umbra-core' ) . '</p>';
		echo '<p>' . esc_html__( 'Choose a NOO (.xml) file to upload, then click Upload file and import.', 'noo-umbra-core' ) . '</p>';
	...
	function _noo_import_data( $data ) {
		...
		if ( empty( $data ) || ! is_object( $data ) ) {
			wp_die(
				esc_html__( 'Import data could not be read. Please try a different file.', 'noo-umbra-core' ),
				'',
				array( 'back_link' => true )
			);
		}
		...
		foreach ( $data as $sidebar_id => $widgets ) {
			...
			if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
				...
				$sidebar_message = esc_html__( 'Sidebar does not exist in theme (using Inactive)', 'noo-umbra-core' );
			}
			...
			foreach ( $widgets as $widget_instance_id => $widget ) {
				...
				if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
					...
					$widget_message = esc_html__( 'Site does not support widget', 'noo-umbra-core' ); // explain why widget not imported
				}
				...
				if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
					...
					foreach ( $single_widget_instances as $check_id => $check_widget ) {
						...
						if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
							...
							$widget_message = esc_html__( 'Widget already exists', 'noo-umbra-core' ); // explain why widget not imported
				...
				if ( ! $fail ) {
					...
					if ( $sidebar_available ) {
						...
						$widget_message = esc_html__( 'Imported', 'noo-umbra-core' );
					} else {
						$widget_message_type = 'warning';
						$widget_message = esc_html__( 'Imported to Inactive', 'noo-umbra-core' );
					}
				...
				$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = ! empty( $widget['title'] ) ? $widget['title'] : esc_html__( 'No Title', 'noo-umbra-core' ); // show "No Title" if widget instance is untitled
	...
	function process_option( $file ) {
		...
		foreach ( (array) $options_to_import as $option_name ) {
			...
			if( $primary_menu = wp_get_nav_menu_object( 'Menu left' ) ) {
                $nav_menu_locations['primary-left'] = $primary_menu->term_id;
            }
			if( $primary_menu = wp_get_nav_menu_object( 'Menu Right' ) ) {
                $nav_menu_locations['primary-right'] = $primary_menu->term_id;
            }
