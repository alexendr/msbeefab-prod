...
class Noo_Notice_Install {
	...
	public function load_enqueue_script_setup() {
		...
		wp_register_script( 'setup-install-demo', NOO_PLUGIN_ASSETS_URI . '/js/min/noo.setup.install.demo.js' );
		...
		wp_localize_script( 'setup-install-demo', 'nooSetupDemo', 
			array( 
				...
				'notice'   => esc_html__( 'Do you want to continue this action?', 'noo-umbra-core' ),
				'warning'  => esc_html__( 'Please waiting, not exit page.', 'noo-umbra-core' ),
	...
	public function notice_html_install() {
		...
		<div id="message" class="updated notice is-dismissible">
			<p><strong><?php echo sprintf( esc_html__( 'Welcome to %s,', 'noo-umbra-core'), wp_get_theme($this->product_id)->get('Name') ); ?></strong></p>
			<p><?php echo esc_html__( 'If it is the first time you install this theme, you should go and check the basic setting.', 'noo-umbra-core'); ?></p>
			<p class="submit">
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=noo-setup' ) ); ?>" class="button-primary">
					<?php echo esc_html__( 'Go to Quick Setup', 'noo-umbra-core' ); ?>
				</a>
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=noo-setup&action=skip' ) ); ?>" class="button">
					<?php echo esc_html__( 'Skip Setup', 'noo-umbra-core' ); ?>
				</a>
			</p>
		</div>
		<?php
	...
	// -- Create page
		public function admin_menus() {
			add_menu_page ( 
				esc_html__( 'Noo Settings', 'noo-umbra-core' ), 
				esc_html__( 'Noo Settings', 'noo-umbra-core' ), 
				'manage_options', 
				'noo-setup', 
		...
		public function noo_setup() {
			...
			// -----
				...
				if( !is_wp_error( $id_page ) ) {
					...
					echo json_encode( array( 'id' => esc_html__( 'Done', 'noo-umbra-core'), 'slug' => $post->post_name ) );
				}
				exit;
		}
	...
	// -- Tab menu
		public function noo_tab_menu( $current = 'general' ) {
			...
			$tabs = array(
		        'general'   => esc_html__( 'Quick Setup', 'noo-umbra-core' ), 
		        'import_demo'  => esc_html__( 'Import Demo', 'noo-umbra-core' )
		    );
	...
	// -- OPTION
		...
		public function noo_general_options() {
			...
			echo esc_html__( 'Cancel', 'noo-umbra-core' );
			echo '</a>';
			echo '</p>';
			...
			public function noo_option_client() {
				...
				?>
                    <table class="widefat" cellspacing="0" id="client">
                        <thead>
                        <tr>
                            <th colspan="3" data-export-label="<?php esc_html_e( 'Automatic Update Theme', 'noo-ivent' ); ?>">
                                <strong>
                                    <?php esc_html_e( 'Automatic Update Theme', 'noo-ivent' ); ?>
                                </strong>
                            </th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr>
                            <td data-export-label="<?php esc_html_e( 'Purchase Code', 'noo-ivent' ); ?>">
                                <?php esc_html_e( 'ThemeForest Purchase Code:', 'noo-ivent' ); ?><br/>
                                <small><?php esc_html_e( '(Optional)', 'noo-ivent' ); ?></small>
                            </td>
                            <td class="help">
                                <a href="#" title="<?php _e( 'This purchase code makes sure that our server recognizes your website thus allows you to download and install the new updates.', 'noo-ivent' ); ?>" class="help_tip"><span class="dashicons dashicons-editor-help"></span></a>
                            </td>
                            <td>
                                ...
                                <span><?php echo sprintf( wp_kses( __( '<a target="_blank" href="%s">How to get License key?</a>', 'noo-ivent' ), noo_umbra_allowed_html() ), 'http://support.nootheme.com/wp-content/uploads/2015/07/HowToGetPurchaseCode.png' ) ?></span>
                            </td>
                        </tr>
                        </tbody>
                    </table>
			...
			public function noo_option_page() {
				...
				?>
					<table class="widefat" cellspacing="0">
						<thead>
							<tr>
								<th colspan="5" data-export-label="<?php echo esc_html__( 'Required Pages', 'noo-umbra-core' ); ?>"><?php echo esc_html__( 'Required Pages', 'noo-umbra-core' ); ?></th>
							</tr>
						</thead>
						<tbody>
							<?php
								...
								$list_pages = array(
									array(
										'title'         => esc_html__( 'Front Page' , 'noo-umbra-core'),
										'content'       => '',
										'page_template' => '',
										'help'          => esc_html__( 'The Homepage', 'noo-umbra-core'),
										'setting'       => array( 
																'group' => '',
																'key'   => 'show_on_front',
																'value' => 'page',
									...
									array(
										'title'         => esc_html__( 'Member' , 'noo-umbra-core'),
										'content'       => '[noo_member_account]',
										'page_template' => '',
										'help'          => esc_html__( 'The main page for all the action of Employer and Candidate', 'noo-umbra-core'),
										'setting'       => array( 
																'group' => 'noo_member',
																'key'   => 'manage_page_id',
									...
									array(
										'title'         => esc_html__( 'Packages' , 'noo-umbra-core'),
										'content'       => '[noo_job_package_list]',
										'page_template' => '',
										'help'          => esc_html__( 'The page for Employer to choose Job Package', 'noo-umbra-core'),
										'setting'       => array( 
																'group' => 'job_package',
																'key'   => 'package_page_id',
									...
									array(
										'title'         => esc_html__( 'Post Job' , 'noo-umbra-core'),
										'content'       => '',
										'page_template' => 'page-post-job.php',
										'help'          => esc_html__( 'The page for Job posting', 'noo-umbra-core'),
										'setting'       => array(),
									),
									array(
										'title'         => esc_html__( 'Post Resume' , 'noo-umbra-core'),
										'content'       => '',
										'page_template' => 'page-post-resume.php',
										'help'          => esc_html__( 'The page for Resume posting', 'noo-umbra-core'),
										'setting'       => array(),
									),
								);
								...
								foreach ($list_pages as $list_page => $page) :
									...
									if( isset( $setting ) && !empty( $setting ) && is_array($setting) && !empty( $setting['key'] ) ) :
										...
										if( $setting_value == null ) :
											...
											if( isset( $setting['value'] ) && $setting['value'] == $setting_value ) {
												...
												if( !empty( $get_page ) && $get_page->post_type == 'page' ) {
													...
													if( ( isset( $page['page_template'] ) && !empty( $page['page_template'] ) && $page['page_template'] == noo_umbra_get_post_meta( $get_id, '_wp_page_template' ) ) 
										...
										if( $setting_result == 'missing' ) {
											echo "	<mark class='error'>" . esc_html__( 'Missing setting', 'noo-umbra-core' ) . "</mark>";
										} elseif( $setting_result == 'false' ) {
											echo "<mark class='error'>" . esc_html__( 'Wrong', 'noo-umbra-core' );
											if( !empty( $page['content'] ) ) {
												echo ' - ' . sprintf( esc_html__( 'Page should contains %s', 'noo-umbra-core' ), $page['content'] );
											} elseif ( !empty( $page['page_template'] ) ) {
												echo ' - ' . sprintf( esc_html__( 'Page should have template %s', 'noo-umbra-core' ), $page['page_template'] );
											}
											echo "</mark>";
										} else {
											echo "<mark class='yes'>" . esc_html__( 'Done', 'noo-umbra-core');
										...
										if( $setting_result !== 'true' && ( !empty( $page['content'] ) || !empty( $page['page_template'] ) ) ) {
											...
											echo '		<span class="correct-setting" data-title="' . $page['title'] . '" data-content="' . $page['content'] . '" data-page-template="' . $page['page_template'] . '" data-setting-group="' . $setting['group'] . '" data-setting-key="' . $setting['key'] . '">' . esc_html__( 'Correct now', 'noo-umbra-core') . '</span>';
											echo '	</div>';
										}
										echo '</td>';
										echo '<td>';
										if( $setting_result != 'true' && isset( $setting['url']) && !empty( $setting['url'] ) ) {
											...
											echo "		<a href='{$setting['url']}' title='{$page['title']}' target='_blank'>" . esc_html__('Edit setting', 'noo-umbra-core') . "</a>";
											echo '	</div>';
										}
										echo '</td>';
										...
										if ( !empty($page['page_template']) ) :
											...
											if( !empty( $get_id ) && !empty( $get_page ) ) {
												...
												echo '<td>';
												echo "	<mark class='yes'>" . esc_html__( 'Done', 'noo-umbra-core') . " - /{$get_page->post_name}/</mark>";
												echo '</td>';
											} else {
												echo "<td data-export-label='{$page['title']}'>{$page['title']}</td>";
												if( isset( $page['help'] ) && !empty( $page['help'] ) ) {
												...
												echo '<td>';
												echo "	<mark class='error'>" . sprintf( esc_html__( 'You need to create a page with template %s', 'noo-umbra-core' ), $page['page_template'] ) . "</mark>";
												echo '</td>';
												echo '<td>';
												echo '	<div class="button button-primary">';
												echo '		<span class="correct-setting" data-title="' . $page['title'] . '" data-content="' . $page['content'] . '" data-page-template="' . $page['page_template'] . '">' . esc_html__( 'Correct now', 'noo-umbra-core') . '</span>';
												echo '	</div>';
												echo '</td>';
												echo '<td></td>';
											}
		...
		public function noo_import_demo_options() {
			...
			$list_demo = array(
				array(
	                'name' => esc_html__( 'Umbra', 'noo-umbra-core' ),
	                'img'  => NOO_PLUGIN_PATH . 'admin/importer/data-demo/umbra/screenshot.jpg',
	                'file' => 'umbra'
	            ),
			);
			...
			?>
				<table class="widefat" cellspacing="0" style="width: 99%;">
					<thead>
						<tr>
							<th colspan="1" data-export-label="<?php echo esc_html__( 'Settings', 'noo-umbra-core' ); ?>">
								<label class="hide_main">
									<?php echo esc_html__( 'Settings', 'noo-umbra-core' ); ?>
								</label>
							</th>
						</tr>
					</thead>
					<tbody id="noo_main_select">
						<tr>
							<td>
								<input type='checkbox' data-id='import_post' id='import_post' value='1' checked /> <?php echo esc_html__( 'Import Post', 'noo-umbra-core' ); ?>
							</td>
						</tr>
						<tr>
							<td>
								<input type='checkbox' data-id='import_nav' id='import_nav' value='1' checked /> <?php echo esc_html__( 'Import Nav Menu', 'noo-umbra-core' ); ?>
							</td>
						</tr>
						<tr>
							<td>
								<input type='checkbox' data-id='import_comment' id='import_comment' value='1' checked /> <?php echo esc_html__( 'Import Comment', 'noo-umbra-core' ); ?>
							</td>
						</tr>
					</tbody>
				</table>
						...
						<div class="themes">
							<?php foreach ($list_demo as $id => $demo) : ?>
								<div class="theme" tabindex="0">
									...
									<span class="more-details" id="install_<?php echo esc_attr( $demo['file'] ); ?>"><?php esc_html_e( 'Install ' .$demo['name'], 'noo-umbra-core' ); ?></span>
									...
									<div class="theme-actions">
										...
										><?php esc_html_e( 'Install Demo', 'noo-umbra-core' ); ?></button>
									</div>
								</div>
							<?php endforeach; ?>
						</div>
    ...
    public function noo_options_fields() {
        ...
        $this->option_metabox[] = array(
            ...
            'id' => 'general_options',
            'title' => esc_html__( 'General Options', 'noo-umbra-core' )
